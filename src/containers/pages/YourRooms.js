...
const mapStateToProps = state => ({
  ...state.rooms,
  title: state.rooms.addRoom.title,
  desc: state.rooms.addRoom.desc
});
...
const mapDispatchToProps = dispatch => ({
  ...
  onUnload: () =>
    dispatch({ type: YOURROOMS_PAGE_UNLOADED }),
  ...
  onChangeTitle: value => {
		...
		if (value.length === 0) {
			...
			dispatch({ type: UPDATE_ROOM_AUTH, key: key, value: value })
		}
	},
  onChangeDesc: value => {
		const key = 'desc';
